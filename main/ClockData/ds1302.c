...
{ 
    ...
    DS1302_transferEnd(dev);
    // for(int i=0;i<7;i++) ESP_LOGI(TAG, "buf[%d]=0x%x",i,buf[i]);
    ...
    if ((dateTime->second > 59) ||
        ...
        (dateTime->dayWeek > 7) ||
        (dateTime->year > 2099)) // (dateTime->year < 2023)
    {
...
{
    if (!DS1302_begin(&ds1302_dev, DS1302_CLK_PIN, DS1302_IO_PIN, DS1302_CE_PIN)) {
